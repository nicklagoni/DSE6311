...
run_lagged_rf_logit <- function(df, percent_cover_cols){
  ...
  # Helper function to train RF with given ntree
  train_rf <- function(formula, data, mtry, ntree) {
    randomForest(formula, data = data, mtry = mtry, ntree = ntree)
  }
  ...
  for (m in mtry_values) {
    for (n in ntree_values) {
      model <- train_rf(formula_asco, df_lag, mtry = m, ntree = n)
  ...
  for (m in mtry_values) {
    for (n in ntree_values) {
      model <- train_rf(formula_fuc, df_lag, mtry = m, ntree = n)
      preds <- predict(model, df_lag)
...
```{r}
flexible_lagged_rf_model <- function(df, percent_cover_cols, logit_transform = FALSE) {
...
```{r}
#Model_results
logit_results<-flexible_lagged_rf_model(clean_data,logit_transform = TRUE,percent_cover_cols = percent_cover_cols)
raw_results<-flexible_lagged_rf_model(clean_data,logit_transform = FALSE,percent_cover_cols = percent_cover_cols)
```
